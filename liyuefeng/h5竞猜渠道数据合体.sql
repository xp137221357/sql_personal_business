...
left join (
	select date_format(tu.crt_time,'%Y-%m-%d') stat_time,count(tu.user_id) reg_count from forum.t_user_event tu 
...
left join (
	select date_format(t.stat_time,'%Y-%m-%d') stat_time,count(distinct t.charge_user_id) new_recharge_count,sum(t.rmb_value) new_recharge_value from (
...
left join (
	select date_format(t.CRT_TIME,'%Y-%m-%d') stat_time,count(distinct t.charge_user_id) recharge_count,sum(rmb_value) recharge_value 
	from (
...
left join (
	select date_format(oi.CRT_TIME,'%Y-%m-%d') stat_time,count(distinct oi.USER_ID) new_bet_count,round(sum(oi.COIN_BUY_MONEY)) new_bet_coins 
...
left join ( 
	select date_format(oi.CRT_TIME,'%Y-%m-%d') stat_time,count(distinct oi.USER_ID) bet_count,round(sum(oi.COIN_BUY_MONEY)) bet_coins  
...
left join (
	select date_format(tpk.create_time,'%Y-%m-%d') stat_time,sum(earn - pay) pk_consume  
...
left join (
	select date_format(tra.create_time,'%Y-%m-%d') stat_time,sum(earn - pay) pk_roulette 
