...
where T.USER_ID is null AND di.ADD_TIME >= @bt and di.ADD_TIME < @et and di.DEVICE_STATUS != -10

union all

# 新增注册用户数
select '新增注册用户数',concat(@bt,'~',@et)  '日期',count(distinct u.user_id) from forum.t_user u 
inner join forum.t_user_event ue on ue.USER_ID=u.USER_ID and ue.EVENT_PARAM like '%mobile_brand%' 
LEFT JOIN forum.t_user_freeze_log t  ON u.USER_ID=T.USER_ID AND t.reason like '%一元活动(刷子)%' and t.OPT_USER='sys'
where T.USER_ID is null AND  u.CRT_TIME >= @bt and u.CRT_TIME <= @et

union all

# 充值1元礼包人数
select '充值1元礼包人数',concat(@bt,'~',@et)  '日期',count(distinct ai.USER_ID) from forum.t_acct_items ai 
...
## and ai.APP_TYPE in ('BYAPP', 'GAME')
and ai.ADD_TIME >= @bt and ai.ADD_TIME <= @et
and urc.crt_time >= @bt and urc.crt_time <= @et
LEFT JOIN forum.t_user_freeze_log t  ON ai.USER_ID=T.USER_ID AND t.reason like '%一元活动(刷子)%' and t.OPT_USER='sys'
where T.USER_ID is null
...
/*
-- 领取人数以及数量
select concat(@bt,'~',@et)  '日期', count(t.user_id) '领取人数',count(t.user_id) '领取数量' 
