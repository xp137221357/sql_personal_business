...
select * from (
	...
	from (
		select 
		date_format(t.stat_date,'%x-%v') stat_date
		from t_stat_reference_time t
		where t.stat_date>=@beginTime
		and t.stat_date<=concat(@endTime,' 23:59:59')
		group by date_format(t.stat_date,'%x-%v')
	) t
	left join (
		select 
		date_format(ta.crt_time,'%x-%v') stat_time,count(distinct u.USER_ID) first_experts_counts
		from forum.t_user u 
	...
	left join(
		SELECT 
		date_format(tr.crt_time,'%x-%v') stat_time,
		...
		INNER JOIN forum.t_expert_apply ta on ter.APPLY_ID=ta.APPLY_ID and ta.`STATUS`=10 and ta.APPLY_TYPE=0
		and ta.MOD_TIME>=@beginTime and ta.MOD_TIME<=concat(@endTime,' 23:59:59')
		and date_format(ta.MOD_TIME,'%x-%v') = date_format(tr.crt_time,'%x-%v') 
		group by stat_time
