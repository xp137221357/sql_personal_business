...
FROM   (SELECT Date_format(t.add_date, '%Y-%m') AS STAT_TIME,
        ...
        WHERE  Date_format(t.add_date, '%Y-%m') >= date_format(@beginTime,'%Y-%m')   
               AND Date_format(t.add_date, '%Y-%m') <= date_format(@endTime,'%Y-%m')   
       ...
       LEFT JOIN (       SELECT t.add_date                    AS STAT_TIME,
                  ...
                  WHERE  t.date_type = 2
                         and t.add_date >= date_format(@beginTime,'%Y-%m')   
                         and t.add_date <= date_format(@endTime,'%Y-%m')   
