...
inner join (
	...
	inner join (select tg.user_id from report.t_partner_group_detail tg where tg.stat_time in ('2017-08') group by user_id) tr on tr.user_id=u2.USER_ID
	group by u.USER_ID
	...
	inner join (select tg.user_id from report.t_partner_group_detail tg where tg.stat_time in ('2017-08') group by user_id) tr on tr.user_id=u.USER_ID
	group by u.USER_ID
