...
select '充值汇总',count(distinct tc.charge_user_id) '充值人数',sum(tc.coins) '充值金币'
...
select '充值新增投注',count(distinct tc.charge_user_id) '充值人数',sum(tc.coins) '充值金币'
from report.t_trans_user_recharge_coin tc
inner join forum.t_user u on tc.charge_user_id=u.USER_ID
...
select '充值老投注用户',count(distinct tc.charge_user_id) '充值人数',sum(tc.coins) '充值金币'
from report.t_trans_user_recharge_coin tc
inner join forum.t_user u on tc.charge_user_id=u.USER_ID
left join(select user_code from report.t_stat_first_recharge_bet tb where tb.CRT_TIME>=@param0 and tb.CRT_TIME<=@param1 group by user_code) tb 
	on u.USER_CODE=tb.USER_CODE 
...
select '兑出汇总',count(distinct tc.user_id) '兑出人数',sum(tc.coins) '兑出金币'
...
select '兑出新增投注',count(distinct tc.user_id) '兑出人数',sum(tc.coins) '兑出金币'
from report.t_trans_user_withdraw tc
inner join forum.t_user u on tc.user_id=u.USER_ID
left join(select user_code from report.t_stat_first_recharge_bet tb where tb.CRT_TIME>=@param0 and tb.CRT_TIME<=@param1 group by user_code) tb 
	on u.USER_CODE=tb.USER_CODE 
...
select '兑出老投注用户',count(distinct tc.user_id) '兑出人数',sum(tc.coins) '兑出金币'
from report.t_trans_user_withdraw tc
inner join forum.t_user u on tc.user_id=u.USER_ID
left join(select user_code from report.t_stat_first_recharge_bet tb where tb.CRT_TIME>=@param0 and tb.CRT_TIME<=@param1 group by user_code) tb 
	on u.USER_CODE=tb.USER_CODE 
