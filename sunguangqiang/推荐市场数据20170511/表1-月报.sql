...
-- 充值钻石以及新增充值钻石
SELECT '充值钻石以及新增充值钻石',concat(@beginTime,'~',@endTime) '时间',
		  dw.period_name_d '起始日期',dw.diamond_user_cnt '充值钻石人数',dw.diamond_recharge_sum '充值钻石金额',
...
SELECT '充值钻石以及新增充值钻石',concat(@beginTime,'~',@endTime) '时间',
        dw.buy_sv_user_cnt '购买服务人数',dw.buy_sv_sum '购买服务金额',
...
select '数据分析购买服务',concat(@beginTime,'~',@endTime) '时间',
case tu.SERVICE_id
    when 2 then '竞猜'
    when 5 then '平局'
    when 8 then '亚盘'
...
SELECT '专家推荐数据',concat(@beginTime,'~',@endTime) '时间',
