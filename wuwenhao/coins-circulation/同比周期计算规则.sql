...
from t_stat_coin_operate t1 where t1.stat_date = date_add(substring(t.stat_time,12,10),interval 1 day)) free_forbidden,
(select t1.fobidden_normal_coins
from t_stat_coin_operate t1 where t1.stat_date = date_add(substring(t.stat_time,12,10),interval 1 day)) coin_forbidden
...
floor(datediff(tc.CRT_TIME,date_add(@beginTime,interval -(datediff(@endTime,@beginTime)+1)*@num day))/(datediff(@endTime,@beginTime)+1))
