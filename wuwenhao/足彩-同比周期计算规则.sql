...
floor(datediff(date(tf.CRT_TIME),date_add(@beginTime,interval -(datediff(@endTime,@beginTime)+1)*@num day))/(datediff(@endTime,@beginTime)+1)) value,

case floor(datediff(date(tf.CRT_TIME),date_add(@beginTime,interval -(datediff(@endTime,@beginTime)+1)*@num day))/(datediff(@endTime,@beginTime)+1))
...
count(distinct if(u.CRT_TIME>=date_add(@beginTime,interval -(datediff(@endTime,@beginTime)+1)*@num day)  and u.CRT_TIME<=concat(@endTime,' 23:59:59'),u.user_id,null)) new_bet_users ,
count(distinct if(u.CRT_TIME>=date_add(@beginTime,interval -(datediff(@endTime,@beginTime)+1)*@num day)  and u.CRT_TIME<=concat(@endTime,' 23:59:59'),tfc.charge_user_id,null)) new_recharge_users,
count(distinct if(u.CRT_TIME<=date_add(@beginTime,interval -(datediff(@endTime,@beginTime)+1)*@num day) ,u.user_id,null)) old_bets_user ,
count(distinct if(u.CRT_TIME<=date_add(@beginTime,interval -(datediff(@endTime,@beginTime)+1)*@num day) ,tt.charge_user_id,null)) old_recharge_users ,
