...
select  '总参与数', count(distinct t.user_id) from game.t_order_item t 
inner join forum.t_user u on u.user_code = t.user_id and u.CLIENT_ID = 'BYAPP' AND U.STATUS = 10
and t.crt_time >= @beginTime and t.crt_time <= @endTime
and u.NICK_NAME in (
'朦',
'Prometheus',
'DJ',
...
select count(distinct r.user_id) as `官充用户数`, round(sum(money)) `充值金额`  from t_user_recharge r join  forum.t_user u on u.USER_ID = r.user_id
...
select count(distinct r.charge_user_id) as `第三方充值用户数`, round(sum(r.rmb_value)) `充值金额`  from t_trans_user_recharge_coin r 
inner join  forum.t_user u on u.USER_ID = r.charge_user_id
...
select count(distinct r.charge_user_id) as `第三方充值用户数`, round(sum(r.rmb_value)) `充值金额`  from t_trans_user_recharge_diamond r 
inner join  forum.t_user u on u.USER_ID = r.charge_user_id
and u.NICK_NAME in (
'朦',
'Prometheus',
...
select '123',sum(ai.CHANGE_VALUE) as `充值钻石数` from FORUM.t_acct_items ai join tmp_user_bet_0610_0709 t on (ai.ACCT_ID = t.user_id
...
select '456',sum(c.coins) `充值金币数` from tmp_log_charge_coin c join forum.t_user u on u.USER_ID = c.charge_user_id
and u.NICK_NAME in (
'朦',
'Prometheus',
'DJ',
