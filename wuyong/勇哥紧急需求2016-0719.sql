-- 朦,Prometheus,DJ,sarah,亚述帝国,纵横四海,孤剑独行,一览众山小,手机商店,走地波王,信中利周成武,明月照尖东,濠江风云,慢悠悠，晃悠悠,名游彩票,名游彩票APP,口袋德州扑克,高尔夫
...
/*
create table tmp_user_bet_0610_0709
select distinct oi.user_id  from game.t_order_item oi left join v_user_boss u on oi.USER_ID = u.USER_CODE
where oi.CHANNEL_CODE = 'GAME' and oi.CRT_TIME >= @beginTime and oi.CRT_TIME < @endTime and u.USER_ID is null
;

ALTER TABLE `tmp_user_bet_0610_0709`
	CHANGE COLUMN `user_id` `user_id` VARCHAR(50) NOT NULL FIRST,
	ADD PRIMARY KEY (`user_id`);

*/
select  '总参与数', count(1) from tmp_user_bet_0610_0709;
