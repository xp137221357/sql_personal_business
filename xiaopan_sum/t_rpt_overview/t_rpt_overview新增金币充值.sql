...
-- 当天
insert into t_rpt_overview (period_type,period_name,device_type,channel_no,app_version,first_bcoin_unum,first_bcoin_amount) 
SELECT 1 period_type,
       ...
       count(DISTINCT u.user_id) first_bcoin_unum,
       ifnull(sum(ai.COST_VALUE), 0) AS first_bcoin_amount
  FROM forum.t_user u
       LEFT JOIN forum.t_user_event e
          ON u.USER_ID = e.USER_ID AND e.EVENT_CODE = 'REG'
       INNER JOIN report.t_stat_first_recharge_coin tc on u.user_id =tc.user_id
