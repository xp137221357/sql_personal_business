BEGIN     
	...
	WHILE CUR_REF_ID<=MAX_REF_ID DO	
      ...
      then
	      ...
	      select * from (
				select 
				date(date_add(date(date_add(now(),interval 12 hour)),interval -12 hour)) STAT_TIME,
				1,
				t.USER_ID,
				...
				from (
					...
					and o.BALANCE_TIME>=date_add(date(date_add(now(),interval 12 hour)),interval -12 hour)
					and o.BALANCE_TIME< date_add(date(date_add(now(),interval 12 hour)),interval 12 hour)
					group by o.item_id  
				) t
			...
			select * from (
				select date_add(date(date_add(now(),interval 12 hour)),interval -12 hour) STAT_TIME,
				1,
				t.user_id,
				...
				t.OTHER_PAY_MONEY
				 from (
					...
					from (
						...
						and tc.crt_time>=date_add(date(date_add(now(),interval 12 hour)),interval -12 hour)
						and tc.crt_time< date_add(date(date_add(now(),interval 12 hour)),interval 12 hour)
	...
	inner join game.t_order_item o on t.USER_ID=o.USER_ID and t.`TYPE`=1 and t.LAST_ORDER_TIME is null and t.STAT_TIME=curdate()
	and o.item_status not in (0,10,-5,-10,200,210)
	group by t.USER_ID
	on duplicate key update  
	LAST_ORDER_TIME = VALUES(LAST_ORDER_TIME);
	...
	inner join game.t_group_ref tf on tf.USER_ID=t.USER_ID and t.`TYPE`=1 and t.REF_ID is null and t.STAT_TIME=curdate()
	on duplicate key update 
