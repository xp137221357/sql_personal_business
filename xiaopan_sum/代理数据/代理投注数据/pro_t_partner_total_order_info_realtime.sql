CREATE DEFINER=`report`@`%` PROCEDURE `pro_t_partner_total_order_info_realtime`()
	LANGUAGE SQL
	NOT DETERMINISTIC
	CONTAINS SQL
	...
	WHILE CUR_REF_ID<=MAX_REF_ID DO	
      ...
      then
	      insert into t_partner_total_order_info (STAT_TIME,USER_ID,ITEM_ALL_MONEY,ITEM_MONEY,PRIZE_ALL_MONEY,PRIZE_MONEY,PROFIT_ALL_COIN,PROFIT_COIN,EFFECTIVE_MONEY,
			...
			select * from (
         select 
			if(count(t.USER_ID)>0,curdate(),null) STAT_TIME,,
			CUR_USER_ID,
			...
			left join report.t_partner_order_info t on t.USER_ID=tu.child_user_id and t.STAT_TIME=curdate()
