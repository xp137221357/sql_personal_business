CREATE DEFINER=`report`@`%` PROCEDURE `pro_t_partner_order_info_realtime`()
	LANGUAGE SQL
	NOT DETERMINISTIC
	CONTAINS SQL
...
BEGIN     
	...
	WHILE CUR_REF_ID<=MAX_REF_ID DO	
      ...
      then
	      ...
	      select * from (
				select date(date_add(date(date_add(now(),interval 12 hour)),interval -12 hour)) STAT_TIME,
				t.USER_ID,
	...
	inner join game.t_order_item o on t.USER_ID=o.USER_ID and t.LAST_ORDER_TIME is null and t.STAT_TIME=curdate()
	and o.item_status not in (0,10,-5,-10,200,210)
	group by t.USER_ID
	on duplicate key update  
	LAST_ORDER_TIME = VALUES(LAST_ORDER_TIME);
