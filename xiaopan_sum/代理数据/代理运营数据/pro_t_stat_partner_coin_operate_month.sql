CREATE DEFINER=`report`@`%` PROCEDURE `pro_t_stat_partner_coin_operate_month`()
	LANGUAGE SQL
	NOT DETERMINISTIC
	CONTAINS SQL
...
select date_format(date_add(@day_time,interval -1 day),'%Y%m') stat_date,
3 stat_type,
...
and u.crt_time>=date_add(date_add(@day_time,interval -1 month),interval 12 hour) and u.crt_time<date_add(@day_time,interval 12 hour)
inner join (
   SELECT 
...
and tc.crt_time>=date_add(date_add(@day_time,interval -1 month),interval 12 hour)
...
select 
date_format(date_add(@day_time,interval -1 day),'%Y%m') stat_date,
3 stat_type,
count(distinct u.USER_ID) old_recharge_counts,
ifnull(sum(tc.coins),0) old_recharge_coins 
from report.t_trans_user_recharge_coin tc
inner join forum.t_user u on tc.charge_user_id=u.USER_ID and u.crt_time<date_add(date_add(@day_time,interval -1 month),interval 12 hour)
inner join (
   SELECT 
       u.user_id,r1.CRT_TIME
	FROM   forum.t_user u
...
inner join forum.t_user u on tc.user_id=u.USER_ID and u.crt_time<date_add(date_add(@day_time,interval -1 month),interval 12 hour)
inner join (
   SELECT 
       u.user_id,r1.CRT_TIME
	FROM   forum.t_user u
...
and o.BALANCE_TIME>=date_add(date_add(@day_time,interval -1 month),interval 12 hour)
...
from game.t_order_item o
inner join forum.t_user u on o.user_id=u.USER_CODE and u.crt_time<date_add(date_add(@day_time,interval -1 month),interval 12 hour)
inner join (
   SELECT 
       u.user_id,r1.CRT_TIME
	FROM   forum.t_user u
...
where t1.CRT_TIME>=date_add(date_add(@day_time,interval -1 month),interval 12 hour) 
...
and t.crt_time>=date_add(date_add(@day_time,interval -1 month),interval 12 hour) and t.crt_time<date_add(@day_time,interval 12 hour)
inner join (
   SELECT 
       u.user_id,r1.CRT_TIME
	FROM   forum.t_user u
...
and tb.crt_time>=date_add(date_add(@day_time,interval -1 month),interval 12 hour) and tb.crt_time<date_add(@day_time,interval 12 hour)
inner join (
   SELECT 
       u.user_id,r1.CRT_TIME
	FROM   forum.t_user u
...
where ai.PAY_TIME>=date_add(date_add(@day_time,interval -1 month),interval 12 hour)
...
and t.`TYPE`=2
where date(t.EXPECT)>=date_add(@day_time,interval -1 month)
...
SET @day_time = date_add(@day_time,interval 1 month);
end while label1;
