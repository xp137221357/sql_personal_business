...
set @param1='2017-07-03 12:00:00';

set @param2=concat(@param0,'~',@param1)

-- select * from t_partner_group t where t.is_valid=0;

select @param2,'汇总',count(distinct o.USER_ID) bet_users,round(sum(o.COIN_BUY_MONEY)) bet_coins,round(IFNULL(sum(o.COIN_PRIZE_MONEY),0)+IFNULL(sum(o.COIN_RETURN_MONEY),0)) prize_coins
...
-- 新增首次
select @param2,'新增',count(distinct o.USER_ID) bet_users,round(sum(o.COIN_BUY_MONEY)) bet_coins ,round(IFNULL(sum(o.COIN_PRIZE_MONEY),0)+IFNULL(sum(o.COIN_RETURN_MONEY),0)) prize_coins
from game.t_order_item o
...
-- 老用户
select @param2,'老用户',count(distinct o.USER_ID) bet_users,round(sum(o.COIN_BUY_MONEY)) bet_coins ,round(IFNULL(sum(o.COIN_PRIZE_MONEY),0)+IFNULL(sum(o.COIN_RETURN_MONEY),0)) prize_coins
from game.t_order_item o
