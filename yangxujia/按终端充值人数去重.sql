...
select count(distinct tt.receiver) from (
...
select '赏金' type_,receiver, sum_in_actual from (
	...
	inner join forum.t_user u0 on oa.USER_ID = u0.USER_CODE -- and u0.CRT_TIME>=@beginTime and u0.CRT_TIME<=@endTime
	union all 
	## 币商被动给下级用户金币
	...
	inner join game.t_offer o on o.OFFER_ID = oa.OFFER_ID and o.OFFER_STATUS = 80 and o.IS_FINISH = 1
      ...
      and o.USER_ID not in (select user_code from test.new_user_boss)
	   inner join forum.t_user u2 on o.USER_ID = u2.USER_CODE -- and u2.CRT_TIME>=@beginTime and u2.CRT_TIME<=@endTime
		)t 
	union all
	# 用户赠送
	SELECT 
	...
	and  tp.status = 10
	      ...
	      inner join forum.t_user u3 on tp.TUSER_ID = u3.USER_id -- and u3.CRT_TIME>=@beginTime and u3.CRT_TIME<=@endTime
      
      	union all
		## 线下网银
	select '线下网银',ai.user_id charge_user_id, ai.change_value as coins
	 from forum.t_acct_items ai
	  inner join forum.t_user u4 on ai.USER_ID = u4.USER_id -- and u4.CRT_TIME>=@beginTime and u4.CRT_TIME<=@endTime
	...
	select t.user_id receiver from test.t_stat_first_recharge t 
	inner join test.t_trans_user_attr tu on tu.USER_ID = t.user_id
	  and tu.SYSTEM_MODEL = @system_model
	 where t.crt_time>=@beginTime and t.crt_time<=@endTime 
	) tt
