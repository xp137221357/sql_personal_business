...
left join (
select tu.CHANNEL_COMPANY,tu.SYSTEM_MODEL,count(distinct oi.user_id) bet_count,round(sum(oi.COIN_BUY_MONEY)) bet_coins from (
select * from (
...
left join game.t_order_item oi on tu.USER_CODE=oi.USER_ID 
...
group by tu.CHANNEL_COMPANY,tu.SYSTEM_MODEL
) t4 on t1.CHANNEL_COMPANY=t4.CHANNEL_COMPANY and t1.SYSTEM_MODEL=t4.SYSTEM_MODEL 