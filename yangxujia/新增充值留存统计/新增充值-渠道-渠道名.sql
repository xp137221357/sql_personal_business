...
date(rb.crt_time) '新增日期', date(o.ADD_TIME) '留存日期', ifnull(count(distinct o.USER_ID),0) '人数',sum(ifnull(o.COST_VALUE,0)) '金额',ch.channel_name '渠道名' FROM test.T_STAT_FIRST_RECHARGE_BET rb
...
inner join forum.t_device_channel ch on ch.CHANNEL_NO = te.CHANNEL_NO and ch.CHANNEL_NAME in ('百盈猜球','百赢足球','百盈竞彩大师','百盈足球助手','百盈足球风云','百盈足球大师','百盈足球彩票')
...
group by date(rb.crt_time), date(o.ADD_TIME),ch.channel_name;
