...
set @endTime = '2016-09-22 23:59:59';
set @channel_company = '今日头条';
...
select @channel_company,concat(@beginTime,'~',@endTime),'官方新增充值人数以及金额',count(distinct ttt.charge_user_id),sum(ttt.rmb_value) from (
...
select @channel_company,concat(@beginTime,'~',@endTime),'非官方新增充值人数以及金额',count(distinct ttt.charge_user_id),sum(ttt.rmb_value) from (
select '金币',tu.SYSTEM_MODEL,tu.CHANNEL_COMPANY, tc.charge_user_id,tc.rmb_value from t_trans_user_recharge_coin tc
inner join t_trans_user_attr tu on tc.charge_user_id = tu.USER_ID and tu.CHANNEL_COMPANY=@channel_company
inner join forum.t_user u on u.USER_ID= tu.user_id and u.CLIENT_ID='BYAPP' and u.crt_time>=@beginTime and u.crt_time<=@endTime
inner join (
...
select @channel_company,concat(@beginTime,'~',@endTime),'总官方新增充值人数以及金额',count(distinct ttt.charge_user_id),sum(ttt.rmb_value) from (
select '金币',tu.SYSTEM_MODEL,tu.CHANNEL_COMPANY, tc.charge_user_id,tc.rmb_value from t_trans_user_recharge_coin tc
inner join t_trans_user_attr tu on tc.charge_user_id = tu.USER_ID and tu.CHANNEL_COMPANY=@channel_company
inner join forum.t_user u on u.USER_ID= tu.user_id and u.CLIENT_ID='BYAPP' and u.crt_time>=@beginTime and u.crt_time<=@endTime
inner join (
